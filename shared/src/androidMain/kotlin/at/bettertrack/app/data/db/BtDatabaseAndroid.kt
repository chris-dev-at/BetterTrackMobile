...
// ─────────────────────────────────────────────────────────────────────────────
// KMP/iOS port (Phase 2): the ANDROID construction of [BtDatabase].
//
// The @Database, entities and DAOs moved to commonMain, but the migration chain
// stayed HERE, byte-identical to what shipped: these nine migrations are battle-
// tested (their KDoc records two real crash-loop incidents) and the migration
// regression suite runs them exactly as [BtDatabase.create] registers them.
// Android keeps building through the classic SupportSQLite path
// (`Room.databaseBuilder(context, BtDatabase::class.java, name)`), which is what
// keeps `db.openHelper` — used by AccountDataManager's scoped wipe — available.
// The migrations, [MIGRATIONS] and [create] hang off `BtDatabase.Companion` as
// extensions so every existing `BtDatabase.create(...)` / `BtDatabase.MIGRATIONS`
 ...
 */
private val MIGRATION_VAULT_TABLES = object : Migration(8, 9) {
    override fun migrate(db: SupportSQLiteDatabase) {
 ...
 */
private val MIGRATION_SYNC_ERROR_CODE = object : Migration(9, 10) {
    override fun migrate(db: SupportSQLiteDatabase) {
 ...
 */
private val ANDROID_MIGRATIONS: Array<Migration> = arrayOf(
    MIGRATION_1_2,
    MIGRATION_2_3,
    MIGRATION_3_4,
    MIGRATION_4_5,
...
/**
 * The registered migration chain — the exact array [create] passes to Room, and
 * the array `BtDatabaseMigrationTest` replays. Exposed on the companion so the
 * `BtDatabase.MIGRATIONS` reference the test already uses keeps resolving now
 * that the chain lives in androidMain rather than the companion body.
 */
val BtDatabase.Companion.MIGRATIONS: Array<Migration> get() = ANDROID_MIGRATIONS

/**
 * The Android factory — byte-identical to what shipped before the KMP move:
 * the classic `Room.databaseBuilder(context, Class, name)` SupportSQLite path
 * with the full historical migration chain and no destructive fallback. Building
 * through this path (rather than the KMP `setDriver` path iOS uses) is what keeps
 * `db.openHelper` available for AccountDataManager's scoped wipe.
 */
fun BtDatabase.Companion.create(context: Context): BtDatabase =
    Room.databaseBuilder(context, BtDatabase::class.java, "bettertrack.db")
        .addMigrations(*ANDROID_MIGRATIONS)
        .build()
